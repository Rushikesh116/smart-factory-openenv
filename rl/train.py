...
def train_ppo(
    ...
    print(f"Best model (eval callback): {os.path.join(best_model_dir, 'best_model.zip')}")
    paths = {
        "task_id": task_id,
        "run_dir": run_dir,
        ...
        "best_model": os.path.join(best_model_dir, "best_model.zip"),
    }
    return paths


def train_curriculum(run_dir: str | None = None, debug_env: bool = False) -> dict:
...
if __name__ == "__main__":
    ...
    if args.mode == "curriculum":
        train_curriculum(run_dir=run_dir, debug_env=args.debug_env)
    else:
        train_ppo(
